...
// ScrollySign -- We took the MarqueeText sketch in the example and modified it a little bit so you can control it over serial port
...
byte font[95][8] = { {0,0,0,0,0,0,0,0}, // SPACE
                     ...
                     {0x42,0xe7,0xff,0xff,0xdb,0xff,0xe7,0x7e}, // *
...
void loop()
{
    if(Serial.available()) {                       // scattered serial reads about as some of these functions block for a period of time
    newtext += String((char)Serial.read());
  }
  ...
  if(Serial.available()) {                        // scattered serial reads about as some of these functions block for a period of time
	...
	if( --x < len * -8 ) {
		x = LEDMATRIX_WIDTH;
    Serial.print("#");                        // Indicates scroll restarted (and is ready for text)
		if(newtext != "") { 
       Serial.print("*");                     // Indicates new message received
       text = String(newtext); 
       newtext = String(""); 
       } 
	}
 ...
 */
void drawString(String text, int len, int x, int y )
{
